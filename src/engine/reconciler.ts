...
export class Reconciler {
  ...
  static hoistHeader(content: string, header: string): string {
    // 1. Remove all existing occurrences of generated code headers and common boilerplate
    const cleanText = content
      .replace(/^\s*\/\/ GENERATED CODE.*$/gm, '')
      .replace(/^\s*\/\/ This file is automatically generated by.*$/gm, '')
      .replace(/^\s*\/\/ Any manual changes will be overwritten.*$/gm, '')
      .trimStart();

    // 2. Prepare header text
    const headerText = header.endsWith('\n') ? header : `${header}\n`;

    return headerText + cleanText;
  }
